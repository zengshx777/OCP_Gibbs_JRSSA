...
rr.model<-function()
{
  for (j in 1:nsample)
  {
    ...
    y1[j]<-d1+inprod(beta1,X[j,])+gamma1*d[j]+y1.tau[rd.ind[j]]
    y0[j]<-inprod(beta0,X[j,])+gamma0*d[j]+y0.tau[rd.ind[j]]
    
    #Categorical Model
    #Caculate the prob in each cell with inverse logit function
    g1[j,1]=ilogit(c[1]+y1[j])
    g1[j,2]=ilogit(c[2]+y1[j])
    g1[j,3]=ilogit(c[3]+y1[j])
    g1[j,4]=ilogit(c[4]+y1[j])
    ...
    g0[j,1]=ilogit(c[1]+y0[j])
    g0[j,2]=ilogit(c[2]+y0[j])
    g0[j,3]=ilogit(c[3]+y0[j])
    g0[j,4]=ilogit(c[4]+y0[j])
...
parameters=c("gamma0","gamma1","d1","theta","beta1","tinter","y1","y0",
             "beta0","d","t.tau","y1.tau","y0.tau","c","taud.prec")
