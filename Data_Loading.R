...
prod.info<-aggregate(data.try$onechild,by=list(data.try$provcd_born),function(x) length(unique(x)))
data.try<-subset(data.try,data.try[,"provcd_born"]%in%prod.info$Group.1[prod.info$x==2])
#extract the cluster with size greater than 10
data.try<-subset(data.try,provcd_born%in%names(table(data.try$provcd_born))[
  table(data.try$provcd_born)>8])
